...
source("http://bioconductor.org/biocLite.R")
## if useDevel() and biocLite("BiocUpgrade")doesn't work, run following code
## useDevel()  
...
API client uploader not implemented yet.

```{r}
a <- Auth(username = "tengfei", platform = "us-staging")
fl <- system.file("extdata", "sample1.fastq", package = "sevenbridges")
fl
(p <- a$project(id = "tengfei/quickstart"))
pid <- p$id
pid
u <- sevenbridges:::Upload(auth = a,
            fl,
            project_id = pid, name = "sample13.fastq")
u$upload_file()
p$member()
