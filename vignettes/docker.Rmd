...
- Q: Can I push a docker hub image to SBG registry?
- A: Yes. make sure you login (sbg login or cgc login), Please rename the tag and do a docker push. for example, if you have a image called runif and logged in as 'yintengfei'. When you do `sbg docker-push` you don't need full name, <repository>[:<tag>] will do the trick, and for now `docker push` don't work on this. 

```
docker tag runif images.sbgenomics.com/yintengfie/runif
sbg login
sbg docker-push yintengfie/runif
```

will do the trick.

Our own image registry:
...
OK seems good, now let's test it in our terminal, don't forget to make it executable by doing something like `cp runif.R /usr/local/bin; chmod 755 runif.R`
...
# Upgraded way: writing executable report with R markdown
...
```{r, eval = TRUE}
fl <- system.file("docker/runif/src/", "runif.R", package = "sevenbridges")
fl
```
...
```{r, comment='', eval = TRUE}
fl <- system.file("docker/runif/report/", "report.Rmd", package = "sevenbridges")
cat(readLines(fl), sep = '\n')
```
...
```{r}
fl = "/Users/tengfei/Code/svnrepos/bioc-devel/sevenbridges/inst/liftr/runif.Rmd"
library(knitr)
library(yaml)
lift.rabix(fl)
rabix.lst
inl <- IPList(lapply(rabix.lst$rabix$inputs, function(i){
    do.call(input, i)
}))

asDoc
## let's convert a inl into docopt command line
