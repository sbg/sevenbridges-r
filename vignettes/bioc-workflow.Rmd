...
4. Desribe a standard RNA-seq Bioconductor workflow in CWL with pre-defined report template
5. Excecute it in the cloud
...
Suggeste learning for all users: docker.
...
For now I recommend you to install the latest version of 'sevenbridges' from github directly by runing the script:
...
You can find login/registration on NCI Cacner Genomics Cloud homepage [http://www.cancergenomicscloud.org/](http://www.cancergenomicscloud.org/), follow up the [signup tutorial](http://docs.cancergenomicscloud.org/docs/sign-up-for-the-cgc) if you have ERA Commons.

### Get your authentifiation

After you login, you can get your authentification under yoru account setting and 'developer' tab ([tutorial])(http://docs.cancergenomicscloud.org/docs/get-your-authentication-token)



### (optional) register on shinyapps.io

In this tutorial, if you want to try to deploy the application automatially on shiny server like shinyapps.io, please viist [http://www.shinyapps.io/](http://www.shinyapps.io/) register and login. 

Get you token and secret ready to deploy

![shiny-token](http://shiny.rstudio.com/images/tokens.png)
...
This packge is under active development, will bring many new features as well, at 
...
The final goal is make a worflow that 

1. Input gene feature, design matrix, bam files, and generate differential experession report and output full report, a picture and a count table as example. 
2. Add report tool with two Shiny app template and two R markdown template to collect files from previsous flow and generate new reprot, even deploy on shinyapps.io autmatically after a task is finished.
...
A full html report included, it's also output from the first tool, in this way, you can orchastrate many tools output into single report for your task.
...
To create Auth, just pass token and url, by default url is setto CGC. Good news you can
use 'sevenridges' package to access any seven bridges platform with V2 API. 
...
Alternatively you can save your authentification cross different platforms in a configuration file called ".sbg.auth.yml" at home folder. This allow you to manage multiple users on multiple platforms. 
...
Now let's create a new project called "hackson", save it to a 'p' object for conventient usage for any call related to this project.
...
1. First you need to check is there any existing container you can directly use, so you don't have to make a new one. If you don't know what to use, I will suggest you start with "rocker/hadleyverse", it has lots stuff markdown, kntir need and other hadley's popular packages. 
...
For example, you know there is 'runif' function in rocker/r-base container, you can just do something like this.  Please read another tutorial called "Describe CWL Tools/Workflows in R and Execution", it introduces you a simpler example with randome number generator. 
...
3. For advnaced developer/users: if you think a cool command line interface is worth doing, and convenient in any case, then go ahead and make one and create our own container will always be a better idea. this allow you to provide much formal interface at both command line level or container level. 
...
you can hard-code your template into your docker container, like you noticed in the Dockerfile I created, I insert a report R markdown tempalte. And in the commadn line 
interface, in the end, I pass it to `rmarkdown::render` function as parameters 'param'. In this way, you define your input in the header of R markdown. Examples like this, this template I used here, is exactly the same report on Bioconductor's Rna-seq worklfow [website](http://www.bioconductor.org/help/workflows/rnaseqGene/)
...
Well, you are always encouraged to use graphical user interacce like this
...
uses two different engines to provide isolated environment to generate report, because each report has different dependendies.
...
2. In this root foler, you have "data", "src", "www", foler, on the interface, you can
...
If you are interest, you can stil read my dockerfile, commandline, tool generator and json. 

__I will suggest you directly copy the json into your project.__
...
Even though in R with 'sevenbridges' package you can use `%>%` to connect two Tool object, but this only works for lineawr simple flow. For complicated flow, we recommend you to use graphic user interface, it's lot fun. 

Now connect your RNA-seq tool with the report tool you just add it to your project, 
...
write report template for your tool (hard coded) and then share your flow with yoru friends. 
