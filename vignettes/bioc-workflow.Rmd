...
prepared, and it's recommended for beginner to read and run through all examples
here yourself in your R IDE like Rstudio. Then try to make your own app.
...
this Rmarkdown in Rstudio. It's easy to learn!
...
First download the [R markdown file](https://raw.githubusercontent.com/sbg/sevenbridges-r/master/vignettes/bioc-workflow.Rmd) of this page, so you can load it to yoru Rstudio 
or your favorite IDE to run through all examples and tweak setup.

This package __sevenbridges__ is already on Bioconductor [devel branch](https://www.bioconductor.org/packages/3.3/bioc/html/sevenbridges.html),
the release day is going to be May 4th, 2016. Please check the detailed [release schedule](https://www.bioconductor.org/developers/release-schedule/).
It is also synced on [GitHub repo](https://github.com/sbg/sevenbridges-r)

For now I recommend you to install the latest version of `sevenbridges-r`
from github directly by running the script:
...
You can find login/registration on NCI Cancer Genomics Cloud homepage [https://www.cancergenomicscloud.org](https://www.cancergenomicscloud.org), follow up the [signup tutorial](http://docs.cancergenomicscloud.org/docs/sign-up-for-the-cgc) if you have ERA Commons.

### Get your authentifiation

After login, you can get your authentication under your account setting and 'developer' tab ([tutorial])(http://docs.cancergenomicscloud.org/docs/get-your-authentication-token)

### (optional) register on shinyapps.io

In this tutorial, if you want to try to deploy the application automatically on shiny server like shinyapps.io, please visit [https://www.shinyapps.io/](https://www.shinyapps.io/) to register and login.
...
any moment, if you have questions or problem about this R package, please file issue on [github issue page here](https://github.com/sbg/sevenbridges-r/issues)
...
Alternatively you can save your authentication cross different platforms in a configuration file at the default path `$HOME/.sevenbridges/credentials`. This allows you to manage multiple users on multiple Seven Bridges environments.
...
When you have this user configuration file ready at this default location, all you need to do is setting `use_config = "???"` and set the `profile_name` to use.

```{r}
a <- Auth(use_config = "???", profile_name = "cgc")
## remove old project
...
For example, you know there is an `runif` function in the `rocker/r-base` container, you can just do something like this.  Please read another tutorial called "Describe CWL Tools/Workflows in R and Execution", it introduces you a simpler example with random number generator.
...
Well, you are always encouraged to use graphical user interface,
but it's at the same time fun to learn how to do it in R, so you could script
how you build it together, like what I did here, for every single tool json,
I have an R script called generator.R in the same folder, so I can always trace
back. 

For example, you can see the same in my github page, under [inst/docker](https://github.com/sbg/sevenbridges-r/tree/master/inst/docker), you will see three examples, 1. package docker 2. rna-seq tool 3. report tool, under each folder, you will see 1. one dockerfile, 2. src/ for command line 3. report/ for report template and 4. rabix a generator file and a json.
