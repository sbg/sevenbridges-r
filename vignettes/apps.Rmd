...
For R users who want to script in everythinghere is alternative solution. I also did some work trying to make it simpler to use, any suggestions are welcomed. 
...
For more accepted parameter, please check `help(CommandLineTool)` and `help(Tool)` for more details. Again we basically conform to CWL standard, you can always check their manua.
...
By defaut the tool object shows YAML, but you can simply convert it to JSON and copy it to your seven bridges platform graphic editor by importing JSON. 
...
With API function, you can direcly load your Tool into the account. Run a task, for "how-to", please check the API complete guide 
...
- Ouput and input match at lest have one match, then we will connect it automatically.
...
tools and the workfow to your project. 
...
a <- Auth(platform = "cgc", username = "tengfei")
p <- a$project("hello")
app.runif <- p$app_add("ramdom_log_mean_new_test_3", f)
aid <- app.runif$id
...
```{r, eval = FALSE}
app1 <- p$app("random number")[[1]]
app2<- p$app("get log")[[1]]
app3 <- p$app("get mean")[[1]]
f <- app1 %>% app2 %>% app3
f$id <- "randome number test flow"
f$toJSON()
```


Workflow interface is still under intensive development, to construct the workflow easily, it's alwasy better to just use our graphical interface, it's much easier for complex worklfow. 
