...
get_uploader = function(
    ...
    if (is.null(destdir))
        stop('Please provide `destdir` to extract the archive')

    platform = match.arg(platform)

    uploader_dict =
        c('cgc'    = 'https://cgc.sbgenomics.com/cgc-uploader/cgc-uploader.tgz',
    ...
    download.file(url = uploader_url, method = 'libcurl',
...
cli_upload = function(
    ...
    dry_run_text = if (dry_run)
        paste('--dry-run', paste0(dry_run_fields, collapse = ' ')) else
            NULL
